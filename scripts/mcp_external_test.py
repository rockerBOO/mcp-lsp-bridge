...
class MCPExternalTestRunner:
    ...
    def parse_command(self, command_string: str) -> Optional[MCPToolCommand]:
        ...
        if not tool_match:
            self._print_colored(
                f"❌ Invalid command format: {command_string}", Colors.RED
            )
            return None
    ...
    def interpolate_variables(
        self, params: Dict, context: "MCPExternalTestRunner.CommandContext"
    ) -> Dict:
            ...
            if (
                ...
                if var_name == "last_result":
                    ...
                    if context.last_result:
                        ...
                        raise ValueError(
                            f"No previous result available for variable {var_name}"
                        )
    ...
    def run_test(
        self,
        command_string: str,
        context: Optional["MCPExternalTestRunner.CommandContext"] = None,
    ) -> Dict:
        ...
        try:
            ...
            interpolated_params = self.interpolate_variables(
                parsed_command.parameters, context
            )
            ...
            if "error" in result:
                self._print_colored(
                    f"❌ Tool call failed: {result['error']}", Colors.RED
                )
            ...
            self._print_colored("✅ Tool call succeeded", Colors.GREEN)
